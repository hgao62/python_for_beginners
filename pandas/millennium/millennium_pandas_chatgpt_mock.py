...
def compute_storage_arbitrage(spot_df, futures_df, storage_cost_per_day):
    merged = spot_df.merge(futures_df, on="Date", suffixes=("_spot", "_futures"))
    merged["Cost_of_Carry"] = (
        merged["Close_futures"] - merged["Close_spot"]
    ) - storage_cost_per_day * (futures_df["Contract_Expiry"] - spot_df["Date"]).dt.days
...
def compute_hedging_pnl(spot_df, futures_df, position_size):
    merged = spot_df.merge(futures_df, on="Date", suffixes=("_spot", "_futures"))
    merged["Hedge_PnL"] = position_size * (
        merged["Close_futures"].diff() - merged["Close_spot"].diff()
    )
