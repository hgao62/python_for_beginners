...
trade_volume_by_ticker = trade.groupby("Ticker")["Quantity"].agg(
    [("Total Volume", "sum"), ("Avg Side", "mean")]
)

trade["net_quantity"] = trade.apply(
    lambda row: row["Quantity"] if row["Trade Type"] == "BUY" else -row["Quantity"],
    axis=1,
)
...
class QuantInterviewChallenges:
    ...
    def challenge_1_daily_returns(self):
        ...
        returns_summary = df.groupby("Ticker")["Returns"].agg(
            [("Mean Return", "mean"), ("Volatility", lambda x: x.std() * np.sqrt(252))]
        )
        returns_summary2 = df.groupby("Ticker").agg(
            {"Returns": ["mean", lambda x: x.std() * np.sqrt(252)]}
        )
    ...
    def challenge_2_trade_analysis(self):
        ...
        net_pos = (
            trade.groupby("Ticker")["net_quantity"].sum().reset_index(name="Net Pos")
        )
        print(net_pos)
        ...
        net_position["Net Position"] = (
            net_position["Total Buys"] - net_position["Total Sells"]
        )
    ...
    def challenge_3_rolling_window(self):
        ...
        stock_data["20D moving average"] = stock_data.groupby("Ticker")[
            "Close"
        ].transform(lambda x: x.rolling(20).mean())
        stock_data["Crossover"] = np.where(
            stock_data["Close"] > stock_data["20D moving average"], "Above", "Below"
        )
    ...
    def challenge_4_performance_attribution(self):
        ...
        perf_summary = trade.groupby("Ticker").agg(
            {"Dollar Volume": ["sum"], "Trading Cost": ["sum", "mean"]}
        )
        ...
        # Daily dollar volume
        self.trades_data["Dollar Volume"] = (
            self.trades_data["Quantity"] * self.trades_data["Price"]
        )

        # Performance summary
        perf_summary = self.trades_data.groupby("Ticker").agg(
            {"Dollar Volume": "sum", "Commission": ["sum", "mean"]}
        )
        perf_summary.columns = [
            "Total Dollar Volume",
            "Total Commissions",
...
def generate_commodity_data():
    ...
    for commodity in commodities:
        for contract in contracts:
            ...
            commodity_data["Basis"] = commodity_data["Price"] * np.random.uniform(
                0.9, 1.1, len(dates)
            )

            # Calculate spread between contracts
            commodity_data["Spread"] = commodity_data["Price"] - commodity_data[
                "Price"
            ].shift(1)
...
class CommodityTradingChallenges:
    ...
    def challenge_1_seasonal_analysis(self):
        ...
        seasonal_analysis = data.groupby(["Month", "Commodity"]).agg(
            {"Price": ["mean", lambda x: x.std()]}
        )
    ...
    def challenge_2_spread_analysis(self):
        ...
        spread_analysis = (
            self.commodity_data.groupby(["Commodity", "Contract"])["Spread"]
            .agg(
                [
                    ("Avg Spread", "mean"),
    ...
    def challenge_3_trading_volume(self):
        ...
        # Volume analysis
        volume_analysis = (
            self.commodity_data.groupby("Commodity")
            .apply(
                lambda x: pd.Series(
                    {
                        "Avg Volume": x["Volume"].mean(),
                        "Volume Price Correlation": x["Volume"].corr(x["Price"]),
                        "Max interest": x["Open Interest"].max(),
                        "Min interest": x["Open Interest"].min(),
                    }
                )
            )
            .reset_index()
    ...
    def calculate_term_structure(self, specific_date=None):
        ...
        for commodity in daily_data["Commodity"].unique():
            ...
            commodity_data["Forward_Rate"] = (
                ...
                / commodity_data["Days_To_Expiry"].diff()
                * 365  # Annualized
            )
            ...
            f = interpolate.interp1d(
                days, prices, kind="cubic", fill_value="extrapolate"
            )
        ...
        for commodity in daily_data["Commodity"].unique():
            ...
            for i in range(len(prices) - 1):
                spread = {
                    ...
                    "Annualized_Spread": (
                        (prices[i + 1] / prices[i]) ** (365 / (days[i + 1] - days[i]))
                        - 1
                    )
                    * 100,
                }
            ...
            spread_analysis[commodity] = {
                ...
                "avg_annualized_spread": np.mean(
                    [s["Annualized_Spread"] for s in spreads]
                ),
            }
    ...
    def plot_term_structure(self, specific_date=None):
        ...
        for commodity, data in term_structure.items():
            ...
            x_smooth = np.linspace(
                raw_data["Days_To_Expiry"].min(), raw_data["Days_To_Expiry"].max(), 200
            )
            plt.plot(
                x_smooth,
                data["curve_fit"](x_smooth),
                label=f"{commodity} (Fitted)",
                linestyle="--",
            )
