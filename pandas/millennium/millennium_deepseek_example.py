...
daily_df = df['Price'].resample('D').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last'})
...
pivot_table = df.pivot_table(values='Price', index='Month', columns='Stock', aggfunc='mean')
...
def example_1():
    ...
    df['Crude_Oil_Volatility'] = df['Crude_Oil_Return'].rolling(window=30).std() * np.sqrt(252)
...
def example_4():
    ...
    df['Significant Change'] = abs(df['Inventory_Change']) > 0.1 * df['Crude_Oil_Inventory'].shift(1)
    print(df.head())
    print("\n")
...
def example_5():
    ...
    df = pd.DataFrame(data)
    df['Rolling correlation'] = df['Gold_Price'].rolling(window=60).corr(df['Silver_Price'])
    print(df.head())
    print("\n")
...
def example_10():
    ...
    for event_date in event_dates:
        pre_event_price = df.loc[df['Date'] == event_date - pd.Timedelta(days=5), 'Wheat_Price'].values[0]
        post_event_price = df.loc[df['Date'] == event_date + pd.Timedelta(days=5), 'Wheat_Price'].values[0]
