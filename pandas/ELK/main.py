...
def enrich_symbol(
    trades: pd.DataFrame, isin_symbol_mapping: pd.DataFrame
) -> pd.DataFrame:
...
def normalize_qty_column(trades: pd.DataFrame) -> pd.DataFrame:
    trades["Quantity"] = trades.apply(
        lambda row: row["Quantity Long"]
        if row["Buy Sell"] == "BUY"
        else row["Quantity Short"],
        axis=1,
    )
    return trades
...
def calculate_pnl(
    trades: pd.DataFrame, clearing_trades: pd.DataFrame, colum_name: str
) -> pd.DataFrame:
    pnl = pd.merge(trades, clearing_trades, on="symbol", how="inner")
    pnl["price_diff"] = pnl.apply(
        lambda row: row["theo"] - row["Trade Price"]
        if row["Buy Sell"] == "BUY"
        else row["Trade Price"] - row["theo"],
        axis=1,
    )
    pnl["price_diff"] = np.where(
        pnl["Buy Sell"] == "BUY",
        pnl["theo"] - pnl["Trade Price"],
        pnl["Trade Price"] - pnl["theo"],
    )
